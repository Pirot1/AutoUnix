...
func SolvingQuiz(page *rod.Page) {
	...
	for i := 0; i < 5; i++ {
		...
		test = append(test, fmt.Sprintf("Question №%d: %s\nVariants:\n%s\n", i+1, question, optionsList))
	}
	var answers []int
	answers = AskGemini_Web(test[:]) //use API if you want
	for i := 0; i < 5; i++ {
		answer := answers[i]
		...
		log.Printf("AI choose: %d", answer)
		if answer == 0 {
			answer = rand.Intn(3) + 1
			log.Printf("AI couldn't generate an answer, therefore the answer was picked randomly: %d\n", answer)
		}
		ans := fmt.Sprintf(`//div[@class="flex flex-col mt-5"]/div[%d]`, answer)
		page.MustElementX(ans).MustClick()
		log.Println("Answer successfully!")
	}
	log.Println("All question was done. Finishing the test up...")
